...
void AES_SubBytes_CPU(BYTE state[], BYTE sbox[]) {
int i;
for(i = 0; i < 16; i++)
    state[i] = sbox[state[i]];   //查表替换
}
...
void AES_AddRoundKey_CPU(BYTE state[], BYTE rkey[]) {
    int i;
    for(i = 0; i < 16; i++)
        state[i] ^= rkey[i];   //与key异或
}
...
void AES_ShiftRows_CPU(BYTE state[], BYTE shifttab[]) {
    BYTE h[16];
    memcpy(h, state, 16);
    int i;
    for(i = 0; i < 16; i++)
...
void AES_MixColumns_CPU(BYTE state[], BYTE AES_xtime[]) {
    int i;
    //ared__ BYTE AES_xtime[];
for(i = 0; i < 16; i += 4) {
    BYTE s0 = state[i + 0], s1 = state[i + 1];
...
void AES_Init_CPU(BYTE AES_Sbox[], BYTE AES_ShiftRowTab[], BYTE AES_Sbox_Inv[], BYTE AES_xtime[], BYTE AES_ShiftRowTab_Inv[]) {
    //__shared__ BYTE AES_ShiftRowTab[16] ;
    AES_ShiftRowTab[0]=0;
    AES_ShiftRowTab[1]=5;
    AES_ShiftRowTab[2]=10;
...
void AES_Encrypt_CPU(aes_block aes_block_array[], BYTE key[], int keyLen, int block_number) {
   int block_index = 0;
   BYTE AES_ShiftRowTab[16];    //共享内存
   BYTE AES_Sbox[256];
   BYTE AES_ShiftRowTab_Inv[16];
   BYTE AES_Sbox_Inv[256];
   BYTE AES_xtime[256];
   ...
   BYTE block[16];  
   for(block_index; block_index<block_number ; block_index++){
        for(int i=0; i<16; i++){
            block[i] = aes_block_array[block_index].block[i];  //不同线程的block，每个block有16个元素
        }
        int l = keyLen, i;  //l=expandkeylen, eg. 16*(10+1) byte
        //printBytes(block, 16);
        AES_AddRoundKey(block, &key[0]);
...
//}
        for(int i=0; i<16; i++){
  //          printf("%d %d  %d\n",i, global_thread_index, aes_block_array[global_thread_index].block[i]);
         aes_block_array[block_index].block[i] = block[i];  //将加密后的block写回各个线程的block中
        }
    }
        //printf("block %d encrypted\n", global_thread_index);   
}
